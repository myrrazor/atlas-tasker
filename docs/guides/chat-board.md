...
The board opens with a horizontal side-scroll Kanban strip. Clear **Side-scroll lanes** to switch to wrapping columns instead; the lane area can be focused and scrolled with a keyboard. Hosts without CSS `:has()` support keep the wrapping layout and hide the switch.
