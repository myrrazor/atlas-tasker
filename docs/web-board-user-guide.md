...
Set the welcome display name with `tracker config set web.owner_name "User"`. Set the workspace language with `tracker config set web.lang ja`; supported values are `en`, `es`, `id`, `zh`, `ja`, and `ko`. Welcome, settings, and board chrome use that preference. Schedule remains English-only, and ticket content stays as written.

The board mirrors Atlas workflow rules. If a dependency, reviewer gate, owner gate, or read-only mode blocks an action, the page shows the service error instead of bypassing policy. Dropping a card on the column it is already in is a no-op on the web board, and the CLI/MCP/bulk paths match that behavior.
