...
Automatic checkpoints use the same restore-safe file set as `backup create`, plus a `.atlas-checkpoint.json` manifest committed in an Atlas-owned bare repository outside the workspace. They append no canonical events. When automatic backup is enabled for an explicit Git target, each checkpoint is published as a fast-forward update of `refs/atlas/backups/<workspace-id>/<replica-id>` (never `refs/heads/*`, never `--force`). Push success is not `verified` until ls-remote plus a fetch into an empty temporary repository confirm the commit, tree, and manifest hash. Verification is remembered per target; `replica reset` clears it. Remote divergence stays `blocked_remote_diverged` until `backup reconcile` or `replica reset`; ticket writes remain available. A failed remote publish does not block restore or compact when a local checkpoint already exists.
