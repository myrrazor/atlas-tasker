...
If signature evidence is present but unreadable, governance explanations include `signature_evidence_unavailable` instead of treating the action as merely unsigned. Atlas also treats duplicate Atlas actor mappings across collaborators as ambiguous, so one raw actor string cannot satisfy collaborator quorum under two identities.
