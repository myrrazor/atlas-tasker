...
Do not describe a change as shipped or stable unless the release evidence says so. `docs/release/public-release-gates.md` is the source of truth. The published installer follows the latest stable GitHub release, and source-only changes remain candidates until tagged and verified. Public identity stays `myrrazor` / GitHub noreply.
