...
func TestStoresLoadV1FixtureWithDefaults(t *testing.T) {
	...
	if !strings.Contains(string(raw), "schema_version: 6") {
		t.Fatalf("expected lazy upgrade write to persist schema_version 6:\n%s", string(raw))
	}
	...
	if !strings.Contains(string(projectRaw), "schema_version: 6") {
		t.Fatalf("expected project update to persist schema_version 6:\n%s", string(projectRaw))
	}
}
