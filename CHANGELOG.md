...
- `tracker board --json` returns `columns`, not `Columns` — the only PascalCase key left in the JSON contract.
...
- README: added the `go install` line, said what `main` has that `v1.9.1` does not, and pointed the agents section at `AGENTS.md`.
